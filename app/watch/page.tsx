...
const VideoContent = () => {
  ...
  const { data } = useSWR(`api/getVideoData/${v}`, fetcher);
  const { viewCount, viewCountShort, likeCount, description, publishDate, uploadTime } = data || 0;
    ...
    <div className="flex w-full justify-between px-10 pt-6">
      <div className="object-fill flex-1 px-6">
        <iframe
          ...
          allowFullScreen
        />
        <h1 ref={titleRef} className="font-bold text-xl mt-2">{videos[v].title}</h1>
